...
pub struct AppState {
    ...
    /// Optional P2P network handle. When present, message routing goes
    /// through the encrypted P2P layer; when absent, HTTP API works in
...
impl AppState {
    ...
    /// Create state backed by a P2P network.
    pub fn with_p2p(p2p: Arc<P2PNetwork>) -> Self {
        Self {
            p2p: Some(p2p),
            ..Self::new()
        }
...
fn registry_error(e: RegistryError) -> ApiResult {
    match &e {
        ...
        RegistryError::TenantExists { .. } => err_json(StatusCode::CONFLICT, &e.to_string()),
        RegistryError::Tenant(_) => err_json(StatusCode::BAD_REQUEST, &e.to_string()),
    }
}
...
pub async fn create_tenant(
    ...
    let result = if let Some(config) = req.config {
        state
            .registry
            .create_tenant_with_config(&req.tenant_id, &req.name, config)
    } else {
...
pub async fn list_tenants(State(state): State<AppState>) -> ApiResult {
    ...
    let data: Vec<serde_json::Value> = tenants
        .iter()
        .map(|t| {
            serde_json::json!({
                "tenant_id": &t.tenant_id, "name": &t.name,
...
pub async fn update_tenant(
    ...
    match state
        .registry
        .update_tenant(&tenant_id, name, active, config)
    {
        Some(t) => ok_json(serde_json::json!({
...
pub async fn deregister_agent(
    ...
    match state.registry.deregister_agent(&tenant_id, &agent_id) {
        Ok(a) => {
            ok_json(serde_json::json!({ "agent_id": a.agent_id, "display_name": a.display_name }))
        }
        Err(e) => registry_error(e),
    }
}
...
pub async fn send_message(
    ...
    if !state
        .rate_limiter
        .try_acquire(&req.tenant_id, &req.from_agent, 1)
    {
    ...
    // Route message via P2P if network handle is available
    let delivered_via_p2p = if let Some(ref net) = state.p2p {
        // Look up the recipient peer by DID in the identity registry
        let peer_id = {
            // Try to parse to_agent as a PeerId, or look up via registry
            let all_peers = net.list_peers().await.unwrap_or_default();
            all_peers
                .into_iter()
        ...
        if let Some(pid) = peer_id {
            // Serialize message as JSON and send encrypted
            let msg = serde_json::json!({
                "from": &req.from_agent,
                "protocol": &req.protocol,
                "payload": &req.payload,
            });
            match net.send_encrypted(pid, serde_json::to_vec(&msg).unwrap_or_default()).await {
                Ok(()) => true,
                Err(e) => return err_json(StatusCode::BAD_GATEWAY, &format!("P2P send failed: {e}")),
            }
        } else {
            false
        }
    ...
    let status = if delivered_via_p2p { "delivered_p2p" } else { "queued_local" };
...
pub async fn list_peers(State(state): State<AppState>) -> ApiResult {
    match &state.p2p {
        Some(net) => match net.list_peers().await {
            Ok(peers) => {
        ...
        None => err_json(StatusCode::SERVICE_UNAVAILABLE, "P2P network not attached"),
    }
}

/// List available resource advertisements from peers.
pub async fn list_resources(State(state): State<AppState>) -> ApiResult {
    match &state.p2p {
        Some(net) => match net.list_resources().await {
            Ok(ads) => {
                let data: Vec<serde_json::Value> = ads
                    .iter()
                    .map(|ad| {
                        serde_json::json!({
                            "agent_id": &ad.agent_id,
                            "spec": {
                                "cpu_cores": ad.spec.cpu_cores,
                                "memory_mb": ad.spec.total_memory_mb,
                            },
                            "cpu_offer": ad.cpu_offer,
                            "memory_offer_mb": ad.memory_offer_mb,
...
pub fn create_router(state: AppState) -> Router {
    Router::new()
        ...
        .route(
            ...
            post(register_agent).get(list_agents),
        )
        .route(
            "/tenants/{tenant_id}/agents/{agent_id}",
            delete(deregister_agent),
        )
...
mod tests {
    ...
    async fn test_duplicate_tenant() {
        ...
        assert_eq!(
            s(&create_tenant(State(st.clone()), Json(req.clone())).await),
            StatusCode::CREATED
        );
        assert_eq!(
            s(&create_tenant(State(st), Json(req)).await),
            StatusCode::CONFLICT
        );
    }
    ...
    async fn test_register_agent() {
        ...
        .await;
        let r = register_agent(
            State(st),
            Path("org-1".into()),
            Json(make_agent("Alice", &["crypto"])),
        )
        .await;
        assert_eq!(s(&r), StatusCode::CREATED);
    ...
    async fn test_send_message() {
        ...
        let r = send_message(
            ...
            Json(SendMessageRequest {
                ...
                to_agent: String::new(),
                tenant_id: "org-1".into(),
                protocol: "data_exchange".into(),
                payload: serde_json::json!({"text": "hello"}),
            }),
        )
        .await;
        assert_eq!(s(&r), StatusCode::OK);
        assert!(r.1 .0["data"]["status"].as_str().unwrap().starts_with("queued") || r.1 .0["data"]["status"].as_str().unwrap().starts_with("delivered"));
    }
    ...
    async fn test_send_message_nonexistent_agent() {
        let r = send_message(
            State(state()),
            Json(SendMessageRequest {
                from_agent: "did:walkie:nope".into(),
                to_agent: String::new(),
                tenant_id: "nope".into(),
                protocol: "text".into(),
                payload: serde_json::json!({}),
    ...
    async fn test_update_tenant_config() {
        ...
        .await;
        for name in &["A", "B", "C"] {
            let rr = register_agent(
                State(st.clone()),
                Path("org-1".into()),
                Json(make_agent(name, &["chat"])),
            )
            .await;
            assert_eq!(
                s(&rr),
        ...
        let r_fail = register_agent(
            State(st),
            Path("org-1".into()),
            Json(make_agent("D", &["chat"])),
        )
        .await;
    ...
    async fn test_send_message_missing_from_agent() {
        ...
        let r = send_message(
            State(st),
            Json(SendMessageRequest {
                from_agent: "did:walkie:notregistered".into(),
                to_agent: String::new(),
                tenant_id: "org-1".into(),
                protocol: "text".into(),
                payload: serde_json::json!({}),
    ...
    async fn test_full_workflow() {
        ...
        assert_eq!(
            s(&register_agent(State(st.clone()), Path("workflow-org".into()), Json(alice)).await),
            StatusCode::CREATED
        );
        assert_eq!(
            s(&register_agent(State(st.clone()), Path("workflow-org".into()), Json(bob)).await),
            StatusCode::CREATED
        );
        ...
        let r4 = send_message(
            State(st.clone()),
            Json(SendMessageRequest {
                from_agent: alice_id.clone(),
                to_agent: bob_id.clone(),
                tenant_id: "workflow-org".into(),
                protocol: "task_assignment".into(),
        ...
        let r5 = send_message(
            State(st.clone()),
            Json(SendMessageRequest {
                from_agent: bob_id.clone(),
                to_agent: alice_id,
                tenant_id: "workflow-org".into(),
                protocol: "status_report".into(),
        ...
        let r7 = update_tenant(
            State(st.clone()),
            Path("workflow-org".into()),
            Json(UpdateTenantRequest {
                name: Some("Updated Workflow".into()),
                active: None,
                config: None,
            }),
        )
    ...
    async fn test_register_agent_duplicate_different_tenant() {
        ...
        let agent = make_agent("Alice", &[]);
        assert_eq!(
            s(&register_agent(State(st.clone()), Path("org-1".into()), Json(agent.clone())).await),
            StatusCode::CREATED
        );
